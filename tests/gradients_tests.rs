...
fn scalar_add() {
    ...
    let y = neuronika::tensor!([1.]).requires_grad();
...
#[test]
fn scalar_sub() {
    let x = neuronika::tensor!([5.]).requires_grad();
    let y = neuronika::tensor!([3.]).requires_grad();
...
fn scalar_div() {
    let x = neuronika::tensor!([5.]).requires_grad();
    let y = neuronika::tensor!([5.]).requires_grad();
