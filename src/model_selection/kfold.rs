...
use super::{Borrow, Cow, Dataset};

pub struct KFold<'a, 'b: 'a, T: Clone> {
    dataset: &'b Dataset<'a, T>,
    splits: usize,
    slice_len: usize,
    iteration: usize,
}

impl<'a, 'b: 'a, T: Clone> KFold<'a, 'b, T> {
    pub fn new(dataset: &'b Dataset<'a, T>, splits: usize) -> Self {
        Self {
            dataset,
            splits,
            slice_len: 1 + (dataset.len() - 1) / splits,
            iteration: 0,
        }
    }
}

impl<'a, 'b: 'a, T: Clone> Iterator for KFold<'a, 'b, T> {
...
mod tests {
    ...
    mod kfold {
        ...
        fn state_transition() {
            ...
            assert_eq!(
                &training[..],
                &[Cow::Owned(0), Cow::Owned(1), Cow::Owned(2), Cow::Owned(3)],
            );
            assert_eq!(
                &test[..],
                &[
            ...
            assert_eq!(
                &training[..],
                &[Cow::Owned(4), Cow::Owned(5), Cow::Owned(6), Cow::Owned(7),],
            );
            assert_eq!(
                &test[..],
                &[
            ...
            let (training, test) = kfold.next().unwrap();
            assert_eq!(&training[..], &[Cow::Owned(8), Cow::Owned(9)],);
            assert_eq!(
                &test[..],
                &[
                    Cow::Owned(0),
    ...
    mod stratified_kfold {
        ...
        #[test]
        fn creation() {
            let dataset = Dataset::new(vec![(1, 1), (1, 2), (2, 1), (2, 2), (3, 1)]);
