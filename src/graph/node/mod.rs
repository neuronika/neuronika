...
use super::{BroadTensor, Broadcasted, Tensor, Var, VarDiff};
use ndarray::{
...
pub struct InputDiff<D: Dimension> {
...
impl<D: Dimension> Gradient for InputDiff<D> {
    type Dim = D;
    ...
    fn was_overwritten(&mut self) {
        debug_assert_eq!(self.can_overwrite, true);

        self.can_overwrite = false;
    }
}

impl<D: Dimension> Backward for InputDiff<D> {
    fn backward(&mut self) -> bool {
        false
    }
}
...
where
    D: Dimension,
{
    type Output = InputDiff<D>;

    fn differentiable(&self) -> Self::Output {
        Self::Output {
            gradient: RefCell::new(Tensor::zeros(self.data.borrow().raw_dim())),
            can_overwrite: true,
        }
    }
}
